...
export default function ChapterSolutions() {
  ...
  return (
    <section id="solutions" ref={section} className="relative bg-surface">
      <div className="mx-auto max-w-[1400px] px-6 pt-32 md:px-10">
        <p className="eyebrow mb-6">
          Chapter 03 - The Future of Sustainable Energy
        </p>
        <h2 className="display-xl mb-4 text-[clamp(2rem,5vw,4rem)] text-ignite">
          Not products. Innovations.
        </h2>
      </div>

      {SOLUTIONS.map((s) => {
        const isSecond = s.id === "saf";
        ...
        return (
          ...
          >
            ...
            <span
              data-num
              className={`pointer-events-none absolute top-1/2 -translate-y-1/2 font-display text-[40vw] leading-none text-vapor opacity-0 md:text-[28vw] ${
                isSecond ? "left-0" : "-right-4"
              }`}
              style={{ color: s.tint }}
            >
              {s.index}
            ...
            >
              <div className={`${isSecond ? "md:order-2 md:text-right" : ""}`}>
                <span
                ...
                <p
                  className={`text-lg leading-relaxed text-muted-vapor ${
                    isSecond ? "ml-auto max-w-md" : "max-w-md"
                  }`}
                >
                  {s.body}
                </p>
              ...
              >
                ...
                <span
                  className={`mt-4 max-w-[16rem] font-mono text-xs uppercase tracking-[0.2em] text-muted-vapor ${
                    isSecond ? "md:text-left" : "md:text-right"
                  }`}
                >
                  {s.metricLabel}
                </span>
